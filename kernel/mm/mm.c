...
{
	mb_mmap_t *mb_mmap = (mb_mmap_t*)mb_info.mmap_addr;
	mb_mmap_t *mb_end = mb_mmap + mb_info.mmap_length;
...
void init_mm(void)
{
	...
	for (i=0; i < mb_info.mods_count; ++i) {
		...
		if (mod->cmdline) {
			printk(KERN_DEBUG "marking cmdline %p %d\n", mod->cmdline, NUM_PAGES(strlen((char*)mod->cmdline)));
			mark_range_used((paddr_t)mod->cmdline, NUM_PAGES(strlen((char*)mod->cmdline)));
		}
	}

	paddr_t sec_addr = (paddr_t)mb_info.u.elf_sec.addr;
	size_t  sec_size = mb_info.u.elf_sec.size;
	size_t  sec_num  = mb_info.u.elf_sec.num;
	...
	printk(KERN_INFO "Total memory:  %dmb\n"
	                 "Usable memory: %dmb\n",
	                 total_mem / MEGABYTE,
